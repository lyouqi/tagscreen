...
//   var myIoSocket = io.connect( configService.server);
//   console.log(configService.server+"/socket");
...
app.service("socketService", function ($websocket, configService) {

  var service = {
      dataStream : $websocket("ws://" + configService.server + "/socket")
  }
...
app.service('utilService', function (configService, $http, $q) {
  ...
  service.get = function (url) {
    ...
    try {
      $http.get("http://" + configService.server + url).success(function (data) {
        if (data.errorCode == 0) {
  ...
  service.post = function (url, data) {
    ...
    var deferred = $q.defer();
    try {
      $http.post("http://" + configService.server + url, data, {
        headers: {
...
app.service("crcService", function(){
  ...
  service.check16 = function(input){
    ...
    for(var i=0;i<input.length;i+=4){
      ...
      crc = ((service.table[(crc ^ parseInt(temp,2)) & 0xff]^(crc >>8)) & 0xffff);
    }
